...
public class StudentCoursesTest extends AbstractCoursesTest {
    ...
    public void itListsAllCoursesForCurrentSemester() throws Exception {
        mockMvc.perform(
                get("/student/lehrveranstaltungen").with(user("student").roles("STUDENT"))
        ).andExpect(
                responseHasCourses(expectedCourses)
        );
    }

    private ResultMatcher responseHasCourses(List<Lehrveranstaltung> courses) {
        return result -> {
            Page<Lehrveranstaltung> page = (Page<Lehrveranstaltung>) result.getModelAndView().getModel().get("allCourses");
    ...
    public void itListsAllCoursesForCurrentSemesterWithNameFilterNoString() throws Exception {
        mockMvc.perform(
                get("/student/lehrveranstaltungen?search=").with(user("student").roles("STUDENT"))
        ).andExpect(
                responseHasCourses(expectedCourses)
        );
    }
    ...
    public void itListsAllCoursesForCurrentSemesterWithNameFilter() throws Exception {
        mockMvc.perform(
                get("/student/lehrveranstaltungen?search=sep").with(user("student").roles("STUDENT"))
        ).andExpect(
                responseHasCourses(singletonList(sepmSS2019))
        );
    }
    ...
    public void onCourseDetailsForStudentWithUnknownCourseItReturnsError() throws Exception {
        mockMvc.perform(
                get("/student/lehrveranstaltungen/100000").with(user(student.getAccount()))
        ).andExpect(
                status().isNotFound()
        );
    }
    ...
    public void itShowsCourseDetailsForStudent() throws Exception {
        mockMvc.perform(
                get("/student/lehrveranstaltungen/" + sepmSS2016.getId()).with(user(student.getAccount()))
        ).andExpect(
